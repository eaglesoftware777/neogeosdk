...
void NEOGEO_USER properties_init(void)
{
    uint8_t g, i;
...
void NEOGEO_USER prop_set(uint8_t group, uint8_t id, int32_t value)
{
...
int32_t NEOGEO_USER prop_get(uint8_t group, uint8_t id)
{
...
void NEOGEO_USER prop_add(uint8_t group, uint8_t id, int32_t amount)
{
    if (group >= NG_PROP_GROUPS) return;
    if (id >= NG_PROP_COUNT) return;
...
void NEOGEO_USER prop_clear_group(uint8_t group)
{
    uint8_t i;
    if (group >= NG_PROP_GROUPS) return;
    for (i = 0; i < NG_PROP_COUNT; i++) {
