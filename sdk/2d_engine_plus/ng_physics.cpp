...
static void NEOGEO_USER ng_physics_apply_drag(int32_t *v, int32_t drag)
{
    if (*v > 0) {
        *v -= drag;
        if (*v < 0) *v = 0;
...
static void NEOGEO_USER ng_physics_resolve_solid(NGCharacter *c, NGPhysicsBody *body, const NGSolidRect *solid)
{
    NGRect old_rect;
    NGRect new_rect;
    NGRect solid_rect;
...
{
    NGPhysicsBody *body;
    uint8_t index = ng_chars_index(c);
