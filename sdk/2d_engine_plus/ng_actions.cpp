...
void NEOGEO_USER ng_actions_init(void)
{
    ...
    for (i = 0; i < NG_MAX_ACTIONS; i++) {
        ng_action_table[i] = nullptr;
    }

    ng_action_sfx_hook = nullptr;
    ng_action_music_hook = nullptr;
    ng_action_fx_hook = nullptr;
}
...
void NEOGEO_USER ng_actions_update(NGCharacter *c)
{
    ...
    while (safety--) {
        ...
        switch (cmd.cmd) {
            ...
            case ACT_FRAME:
                /*
                 * Only force an SCB1 upload when the tile pattern changes.
                 * Re-uploading every action tick causes unnecessary VRAM
                 * traffic and makes flicker more likely.
                 */
