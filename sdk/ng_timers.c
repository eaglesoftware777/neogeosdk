...
static uint8_t ng_timer_percent_value(uint16_t value, uint16_t max)
{
    uint8_t percent = 0;
    uint16_t step = 0;
    uint32_t accum = 0;
...
void NEOGEO_USER timers_init(void)
{
    uint8_t i;
...
void NEOGEO_USER timer_start(uint8_t id, uint16_t frames)
{
...
void NEOGEO_USER timer_stop(uint8_t id)
{
    if (id >= NG_MAX_TIMERS) return;
...
void NEOGEO_USER timers_update(void)
{
    uint8_t i;
    for (i = 0; i < NG_MAX_TIMERS; i++) {
...
uint8_t NEOGEO_USER timer_active(uint8_t id)
{
    if (id >= NG_MAX_TIMERS) return 0;
    return ng_timers[id].active;
}

uint8_t NEOGEO_USER timer_done(uint8_t id)
{
    if (id >= NG_MAX_TIMERS) return 1;
    return ng_timers[id].active == 0;
}

uint16_t NEOGEO_USER timer_value(uint8_t id)
{
    if (id >= NG_MAX_TIMERS) return 0;
    return ng_timers[id].value;
}

uint8_t NEOGEO_USER timer_percent_left(uint8_t id)
{
    if (id >= NG_MAX_TIMERS) return 0;
