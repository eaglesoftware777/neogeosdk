...
void NEOGEO_USER npcs_init(void)
{
    uint8_t i;
...
void NEOGEO_USER npc_remove(NGNpc *npc)
{
    NGCharacter *c;
...
uint8_t NEOGEO_USER npc_count(void)
{
    uint8_t i;
    uint8_t count = 0;
...
void NEOGEO_USER npc_set_think(NGNpc *npc, NGNpcThink think, uint16_t interval_frames)
{
    if (!npc) return;
    npc->think = think;
...
void NEOGEO_USER npc_set_home(NGNpc *npc, int16_t x, int16_t y)
{
    if (!npc) return;
    npc->home_x = x;
    npc->home_y = y;
...
void NEOGEO_USER npc_set_patrol_bounds(
...
void NEOGEO_USER npc_think_patrol(NGNpc *npc, NGCharacter *c)
{
    if (!npc || !c) return;
...
void NEOGEO_USER npc_think_hover(NGNpc *npc, NGCharacter *c)
{
    if (!npc || !c) return;
...
void NEOGEO_USER npcs_update(void)
{
    uint8_t i;
