...
static uint8_t ng_game_events_next(uint8_t v)
{
    v++;
...
void NEOGEO_USER game_events_init(void)
{
...
void NEOGEO_USER game_events_set_handler(NGGameEventHandler handler)
{
    ng_game_events_handler = handler;
}

uint8_t NEOGEO_USER game_events_send(uint16_t id, uint16_t a, uint16_t b, uint16_t c)
{
...
uint8_t NEOGEO_USER game_events_read(NGGameEvent *out)
{
...
void NEOGEO_USER game_events_update(void)
{
    NGGameEvent e;
    while (game_events_read(&e)) {
...
uint8_t NEOGEO_USER game_events_count(void)
{
    uint8_t count = 0;
