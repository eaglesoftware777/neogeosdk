...
This repository carries a reusable runtime layer under `sdk/ng_*`.
...
- event queue
- border constraints
- one per-frame runtime interrupt/update entry point
...
Initialize the runtime once during game startup:
...
Call the runtime once per frame after `waitVbl()`:
...
`game_interupt()` runs this sequence:
...
You can inject high-level callbacks around the runtime pass:
...
That keeps the runtime layer independent from a specific game sound policy.
...
The runtime source files are present in the repository, and the linker scripts
already reserve `runtime_bss` for `out/ng_*0.o`, but the default makefiles do
not compile `sdk/ng_*.c` automatically yet.

To enable the runtime layer in the current SDK build, follow:
