...
static void NEOGEO_USER spr_physics(void)
{
    ...
    /*
     * Screen IDs are generated by artbox order (1-based), not by sprite filename number.
     * Keep these aligned with sprite_meta/out.srt:
     *  - sprite_073_r07_c08 -> screen 75
     *  - sprite_076_r07_c11 -> screen 78
     *  - sprite_077_r07_c12 -> screen 79
     *  - sprite_078_r07_c13 -> screen 80
     */
    ...
    if (eagle) {
        uint16_t base = DEMO_SCREEN_TILE(s_eagle_ground_frame);
        uint8_t strips = demo_screen_strips(s_eagle_ground_frame);
        uint8_t rows = demo_screen_rows(s_eagle_ground_frame);
        uint16_t tile_start = base;
        uint16_t tile_end = (uint16_t)(base + ((uint16_t)(rows - 1u) * 16u) + (uint16_t)(strips - 1u));
        uint8_t fi;

        for (fi = 0u; fi < 3u; fi++) {
            uint16_t t_base = DEMO_SCREEN_TILE(s_eagle_frames[fi]);
            uint8_t t_strips = demo_screen_strips(s_eagle_frames[fi]);
            uint8_t t_rows = demo_screen_rows(s_eagle_frames[fi]);
            uint16_t t_end = (uint16_t)(t_base + ((uint16_t)(t_rows - 1u) * 16u) + (uint16_t)(t_strips - 1u));

            if (t_base < tile_start) tile_start = t_base;
            if (t_end > tile_end) tile_end = t_end;
        }
        ...
        eagle->sprite_offset_y = -(int16_t)((int16_t)demo_screen_rows(frame_id) * 16);
    ...
    for (t = 0u; t < DEMO_END; t++) {
        if (eagle) {
            ...
            if (eagle->sprite_tile        != DEMO_SCREEN_TILE(frame_id)       ||
                ...
                eagle->sprite_active_rows != demo_screen_rows(frame_id)) {
                ng_char_set_sprite(eagle,
                                   eagle->sprite_first,
                                   demo_screen_strips(frame_id),
                                   demo_screen_rows(frame_id),
                                   DEMO_SCREEN_TILE(frame_id),
