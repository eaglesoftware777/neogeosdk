...
ASM_START0
	...
	"\t.equ	REG_PALBANK0,	0x3A000F\t\n"
	"\t.equ	REG_PALBANK1,	0x3A001F\t\n"
...
uint16_t id __attribute__ ((section ("neogeo_init"))) = 			0x0780;
...
/*
 * Software DIP settings: the operator's options for this game, shown in
 * an arcade board's settings menu. All three regions share one table (the
 * header points at it three times); a console always uses the defaults.
 * Layout: the game's name (16 bytes); two time and two count settings,
 * all unused (0xFF); up to ten options, each a byte with its default in
 * the upper four bits and its number of choices in the lower four; then
 * 12-byte, space-padded strings: each option's name followed by its
 * choices. The game reads the chosen values through ng_dip_option().
 */
struct SOFT_DIP_TABLE {
	uint8_t game_name[16];
	uint8_t special_list[6];
	uint8_t option_list[10];
	char    strings[22][12];
};
struct SOFT_DIP_TABLE SOFT_DIPS __attribute__ ((section ("neogeo_gamedata"))) = {
	{'M','A','I','Y','A',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '},
	{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
	{
		0x25,   /* LIVES        1 2 3 4 5         default 3      */
		0x24,   /* CONTINUES    NONE 1 3 5        default 3      */
		0x14,   /* DIFFICULTY   EASY .. EXPERT    default NORMAL */
		0x02,   /* DEMO SOUND   WITH WITHOUT      default WITH   */
		0x02,   /* HOW TO PLAY  WITH WITHOUT      default WITH   */
