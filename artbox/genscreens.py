...
for spec in image_specs:
    cols = int(spec.get("canvas_width", sz * 16)) // 16
    rows = int(spec.get("canvas_height", sz * 16)) // 16
    first = int(spec.get("tile_base", spec["db_index"] * 256))
    tile_map = np.arange(first, first + rows * cols, dtype=np.uint16).reshape(rows, cols)
    maps.append(tile_map)

sys.stdout = SCREENS_C_PATH.open("wt", encoding="utf-8")

# Marks where the generated region begins.  sync_main_screens.py splices this
# whole file into a game's main.c and has to cut the previous copy back out
# first; it cuts here.  Anchoring on showScreen1 instead was fine while every
# generated line lived inside a function, but the palette tables below are at
# file scope and sit ahead of it, so each run kept the last run's tables and
# appended another set.
print(GENERATED_MARKER)

for spec in image_specs:
    sid = spec["screen_id"]
    for i, palette in enumerate(spec.get("extra_palettes", [])):
        values = ",".join(f"0x{word:04x}" for word in palette)
        print(f"static const uint16_t ng_extra_pal_{sid}_{i}[16] = {{{values}}};")
    if "tile_palette_banks" in spec:
        values = ",".join(str(bank) for bank in spec["tile_palette_banks"])
        print(f"static const uint8_t ng_tile_pals_{sid}[] = {{{values}}};")
...
for spec in image_specs:
    ...
    crt_sz, sprt_sz = maps[image_i0].shape
    ...
    print("load_palettes(pal%d,PALETTES+PALOFFSET*%d);" % (image_index, palette_bank))
    for i, slot in enumerate(spec.get("palette_slots", [palette_bank])[1:]):
        print(f"ng_palette_load_bank({slot}, ng_extra_pal_{image_index}_{i});")
    if "tile_palette_banks" not in spec:
        print("uint16_t SCB1_2common = setSCB1_2(%d,0,0,0,0,0);" % palette_bank)
    for sprt_index in range(sprt_sz):
        bank_map = spec.get("tile_palette_banks")
        values = [f"0x{bank_map[row * sprt_sz + sprt_index] << 8:04x}"
                  if bank_map else "SCB1_2common" for row in range(crt_sz)]
        print(f"uint16_t spal{image_index}_{sprt_index + 1}[{crt_sz}]={{{','.join(values)}}};")
...
print("    if (!ng_palette_load_asset(ng_screen_palette_assets, ng_screen_palette_count, screen_id)) return 0;")
print("    switch (screen_id) {")
for spec in image_specs:
    if not spec.get("extra_palettes"):
        continue
    sid = spec["screen_id"]
    print(f"    case {sid}:")
    for i, slot in enumerate(spec["palette_slots"][1:]):
        print(f"        ng_palette_load_bank({slot}, ng_extra_pal_{sid}_{i});")
    print("        break;")
print("    default: break;")
print("    }")
print("    return 1;")
...
for spec in image_specs:
    ...
    print(
        ...
        % (
            ...
            (f"ng_tile_pals_{image_index} + {tile_row * tile_stride + tile_col}"
             if "tile_palette_banks" in spec else "0"),
        )
    )
print("};")
