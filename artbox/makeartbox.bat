...
REM Only the inputs are shared: the tables, palettes and C/S ROMs below are all
REM still generated into this game's own artbox under its own GAME_ID.
set ART_SRC_GAME=%GAME_ART_FROM%
if "%ART_SRC_GAME%"=="" set ART_SRC_GAME=%GAME%
set GAME_ARTBOX_IN=%REPO_DIR%\games\%ART_SRC_GAME%\artbox\in
set GAME_ARTBOX_INFIX=%REPO_DIR%\games\%ART_SRC_GAME%\artbox\infix
set ARTBOX_IN_DIR=%GAME_ARTBOX_IN%
set ARTBOX_INFIX_DIR=%GAME_ARTBOX_INFIX%
if not "%ART_SRC_GAME%"=="%GAME%" echo Art pipeline [%GAME%]: source art from games\%ART_SRC_GAME%\artbox
...
if not exist "%GAME_ARTBOX_IN%" mkdir "%GAME_ARTBOX_IN%"
if not exist "%GAME_ARTBOX_INFIX%" mkdir "%GAME_ARTBOX_INFIX%"
...
(
  ...
  echo /* NEOGEO_USER places this in the section the ROM link keeps; without
  echo  * it the stub lands in .text, which is stripped, and user.c fails
  echo  * to resolve it. */
  echo void NEOGEO_USER showEyeCatcherMVS^(void^) {}
) > "%GAME_EYECATCHER_C%"
